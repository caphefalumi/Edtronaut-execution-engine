...
### Components
1.  **API Gateway (ElysiaJS)**: Handles HTTP requests, manages sessions in MySQL, and pushes execution jobs to Redis. Served on port `1409`.
...
4.  **Database (MySQL)**: Stores persistent state (Sessions & Executions). Containerized alongside the application to support concurrency and data persistence.
...
-   **Database**: Using **MySQL** (containerized) supports multiple API/Worker nodes concurrently, removing the single-file limitation of SQLite.
...
-   **Containerized MySQL**: Adds network latency compared to local SQLite, but enables horizontal scaling and better data integrity.
